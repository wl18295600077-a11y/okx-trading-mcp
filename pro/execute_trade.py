...
def check_filters(ex, inst_id, side: str, params: dict, verbose=True):
    """入场过滤器 — RSI 阈值(硬条件) + EMA50/量比(仅参考提示)。

    2026-08-20 数据拍板: walk_forward 对比 (SATS, 训练/验证/最近30天):
      A 无过滤          训练$+0.47 PF1.62 → 验证$+0.22 PF2.52 ✅
      B 仅EMA50         训练$-0.04 → 验证$-0.11 ❌
      C 仅量比>=1       训练$-0.05 → 验证$+0.05 ❌
      D EMA50+量比(旧)  训练$-0.16 → 验证$-0.11 ❌
    过滤砍掉盈利单(56→26笔), RSI 从 +$0.85 变 -$0.17。EMA50/量比降为参考。
    """
    ...
    if side == "long":
        ...
        checks = [(ok, f"RSI {ind['rsi']:.1f} < {params.get('rsi_low', 35)} (超卖做多)")]
    else:
        ok = ind["rsi"] > params.get("rsi_high", 65)
        checks = [(ok, f"RSI {ind['rsi']:.1f} > {params.get('rsi_high', 65)} (超买做空)")]

    # 参考信息 (不阻断): EMA50 方向 + 量比
    if ind.get("ema_rising") is not None:
        ema_desc = f"EMA50 {'上升' if ind['ema_rising'] else '下降'} ({ind['ema50']:.10f})"
        checks.append((True, ema_desc))
    if ind.get("vol_ratio") is not None:
        checks.append((True, f"量比 {ind['vol_ratio']:.2f}x"))

    all_ok = all(ok for ok, _ in checks)
    if verbose:
        print("  " + " | ".join(("✅ " if ok else "❌ ") + desc for ok, desc in checks))
        print("  (EMA50/量比仅参考，不阻断开仓 — 2026-08-20 数据拍板)")
    return all_ok, ind
...
def place_ladder_orders(ex, inst_id, side, entry_price, sz, params):
    ...
    sl_px = align_tick(ex, inst_id, entry_price * (1 - params["sl_pct"]) if side == "long" else entry_price * (1 + params["sl_pct"]))
    tp1_px = align_tick(ex, inst_id, entry_price * (1 + params["tp1_pct"]) if side == "long" else entry_price * (1 - params["tp1_pct"]))
    ...
    # SL 全量（一张单覆盖全部）
    attach.append({
        "slTriggerPx": fmt_px(ex, inst_id, sl_px),
        "slOrdPx": "-1",
        "sz": str(sz),
        "side": trade_side,
        "posSide": pos_side,
    })
    # 注: TP1 不放 attachAlgoOrds — OKX 会把附加单 sz 归一为开仓全量(阶梯退化fixed)
    return attach, sz_remain, tp1_px, sz_tp1


def align_tick(ex, inst_id, px):
    """价格对齐到 OKX tickSz (避免 51000 tpTriggerPx/tickSz 精度错误)"""
    m = ex.market(inst_id) if inst_id in ex.markets else ex.market(to_ccxt_symbol(inst_id))
    tick = float(m.get("info", {}).get("tickSz", 0.000001))
    if tick <= 0:
        return px
    return round(px / tick) * tick


def fmt_px(ex, inst_id, px):
    """价格格式化为十进制字符串 (OKX 拒绝科学计数法如 '1.16e-08')"""
    m = ex.market(inst_id) if inst_id in ex.markets else ex.market(to_ccxt_symbol(inst_id))
    tick = float(m.get("info", {}).get("tickSz", 0.000001))
...
def place_fixed_orders(ex, inst_id, side, entry_price, sz, params):
    ...
    sl_px = align_tick(ex, inst_id, entry_price * (1 - params["sl_pct"]) if side == "long" else entry_price * (1 + params["sl_pct"]))
    tp_px = align_tick(ex, inst_id, entry_price * (1 + params["tp_pct"]) if side == "long" else entry_price * (1 - params["tp_pct"]))
    trade_side = "buy" if side == "long" else "sell"
    pos_side = "long" if side == "long" else "short"
    ...
    attach = [{
        "slTriggerPx": fmt_px(ex, inst_id, sl_px),
        "slOrdPx": "-1",
        "tpTriggerPx": fmt_px(ex, inst_id, tp_px),
        "tpOrdPx": "-1",
        "sz": str(sz),
        "side": trade_side,
        "posSide": pos_side,
...
def cmd_open(args):
    ...
    if args.mode == "ladder":
        attach, sz_remain, tp1_px, sz_tp1 = place_ladder_orders(ex, inst_id, args.side, price, sz, params)
    else:
        attach, sz_remain = place_fixed_orders(ex, inst_id, args.side, price, sz, params)
        tp1_px, sz_tp1 = None, 0
    ...
    try:
        ...
        # TP1 单独挂条件单（attach 的 TP 会被 OKX 归一为全量, 阶梯必须独立挂）
        if args.mode == "ladder" and sz_tp1 > 0:
            try:
                algo = ex.private_post_trade_order_algo({
                    "instId": inst_id, "tdMode": "isolated",
                    "side": trade_side, "posSide": pos_side,
                    "sz": str(sz_tp1), "ordType": "conditional",
                    "tpTriggerPx": fmt_px(ex, inst_id, tp1_px), "tpOrdPx": "-1",
                    "reduceOnly": "true",
                })
                print(f"  ✅ TP1 条件单已挂 ({sz_tp1}张 @ {tp1_px:.6f})")
            except Exception as e:
                print(f"  ⚠️ TP1 挂单失败: {e}")
    except Exception as e:
        print(f"  ⚠️ 账本记录失败: {e}")
    ...
    if args.mode == "ladder" and sz_remain > 0:
        try:
            ...
            algo = ex.private_post_trade_order_algo({
                ...
                "callbackRatio": str(params["trail_pct"]),  # 回调小数形式 0.03=3% (OKX 范围 0.001-1)
...
def cmd_close(ex=None, inst_id=None, args=None):
    ...
    if not pos:
        ...
        for p in pos:
            ...
            order = ex.create_order(ccxt_sym, "market", "sell" if side == "long" else "buy", sz, None, {"tdMode": "isolated", "posSide": side, "reduceOnly": "true"})
            print(f"  ✅ 已市价平仓 {side} {sz}张")
            # 账本记录平仓 (盈亏自动计算, 含开平仓手续费)
            try:
                exit_px = order.get("average") or (order.get("info") or {}).get("avgPx")
                ledger.record_close(
                    symbol=inst_id,
                    exit_px=exit_px,
                    fee=float((order.get("fee") or {}).get("cost") or 0),
